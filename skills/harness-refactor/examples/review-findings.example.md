...
- Action shape: `move` plus `boundary test`
...
- Route to `harness:lint-test-design`: Yes. Add an invariant that runtime code cannot import provider SDK shapes directly and require changed-code coverage to prove the boundary stays normalized.
...
- Action shape: `move` and `hardgate candidate`
...
- Route to `harness:lint-test-design`: Yes. Add a file-classification invariant, naming rule, or import rule so workflow-owned modules cannot accumulate under generic shared folders.

Merge guidance: acceptable only with focused follow-up or scope narrowing. Do not widen this PR into a broader cleanup.

Commit-time gate judgment for the current staged snapshot: `must_refactor`.
...
Proof expectation for this hotspot: the cleanup should land with one boundary test proving runtime code never reads provider-native fields directly and with full changed-code coverage for the moved parsing path and its immediate orchestration callers.
